...
Added 2026-09-04. Design in
`docs/superpowers/specs/2026-09-04-peptide-shop-design.md`; real sample data for
building against in `docs/shop-sample-payload.json`.

Seven SKUs. Read-only at this stage — there is no cart, no checkout and no order
table yet. Those are blocked on the shop's legal entity and business bank
account existing, not on code.

### Modules

| Path | What |
|---|---|
| `src/lib/shop/types.ts` | `ShopProduct`, `ShopLot`, `AssayState`, `AssayType`, `Component` |
| `src/lib/shop/catalogue.ts` | `PRODUCTS` (7), `LOTS` (9), `lotsFor()`, `currentLot()`, `assayHistory()` |
| `src/lib/shop/pricing.ts` | `perMgCents()`, `formatPrice()`, `formatUnitPrice()` |
| `supabase/shop_schema.sql` | `shop_products`, `shop_lots`, `shop_inventory` — **not applied** |
| `supabase/shop_seed.sql` | generated by `scripts/generate-shop-seed.mjs` |

`catalogue.ts` is the source of truth and the seed is output. Edit the catalogue,
regenerate, commit both.

### The two assay shapes

`assayType: 'purity'` — a single compound. One percentage, `purityPct`.

`assayType: 'composition'` — a blend. The lab reports milligrams per component
and there is **no single percentage to quote**. `components[]` plus
`measuredTotalMg`, read against `labelMg`. KLOW is the only one today: four
actives totalling 90.65 mg against an 80 mg label.

### The three assay states

`assayed` carries the figure its type requires. `pending` carries
`assayExpectedAt` (YYYY-MM) and no figure at all. `none` never lists.

`assayState` is never null, and a database constraint enforces each of those.

### Rules a shop frontend inherits

1. **No Janoshik report code is available, ever.** There is no field for one, no
   column for one, and no endpoint returns one. A code resolves to a public page
   naming the client, the manufacturer and a supplier-prefixed batch — publishing
   one publishes the supply chain. Do not add a "verify" link; there is nothing
   to link to until the assays are commissioned in Peptide Cortex's own name.
2. **`formatUnitPrice()` returns null for a blend.** That is a decision, not
   missing data. A price per milligram of a four-molecule mixture is meaningless
   and ambiguous besides — per 80 mg labelled, or per 90.65 mg measured? Render
   the component table in its place.
3. **Never sort, rank, filter or badge the catalogue by unit price.** Per-mg
   compares *within* a compound: it exists so a buyer can weigh our retatrutide
   against someone else's. NAD+ at $0.075/mg beside Semax at $6.00/mg is not
   eighty times the value. `pricing.ts` exports no comparator and a test fails if
   one is added — that is deliberate, not an oversight to fix. Order by
   `sortOrder`.
4. **`pending` must show its month and must not look comfortable.** It carries
   the line *"This product ships with no published assay until then."* If that
   state reads as tidy, it costs nothing to stay pending and the published
   figures elsewhere stop signalling anything.
5. **`lotCode: null` means no recall path.** Four batches carry it — the vials
   are real, the codes were never recorded. Not an empty string, not a dash.
6. **Stock levels are not exposed.** `shop_inventory` has no read policy.
   Scarcity urgency is the opposite of this shop's positioning.
7. **Every numeric renders in JetBrains Mono** — purity, mg, price, lot code,
   date. Same rule as the rest of the app (CLAUDE.md §16.3).

### Read path

`PRODUCTS` and `LOTS` are typed constants, so the catalogue needs no database
round trip to render. `shop_products` / `shop_lots` exist because orders will
need real foreign keys for lot-level recall traceability — `order_items.lot_id`
is the difference between notifying eleven customers and emailing the whole list.
