...
// Cortex AI chat (Pro): a bench tool.
//
...
export default async function AiChatLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) redirect(loginUrl(currentPath('/ai-chat')))
  return (
    <LibraryChrome signedIn>
      <div className="mx-auto max-w-4xl px-4 pb-8 pt-6 font-sans md:px-6">{children}</div>
    </LibraryChrome>
  )
}
