...
export default function SitesPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#1A1915] flex items-center gap-2">
            <MapPin className="w-6 h-6 text-[#1A8A9E]" />
            Injection Site Tracker
          </h1>
          <p className="text-[#B0AAA0] mt-1">Rotate injection sites and log each session.</p>
        </div>
      </div>

      {/* ── Rotation Suggestion Banner ── */}
      {rotationSuggestion && (
        <div className="flex items-start gap-3 bg-[#1A8A9E]/10 border border-[#1A8A9E]/30 rounded-xl px-5 py-4">
          <RotateCcw className="w-5 h-5 text-[#1A8A9E] mt-0.5 shrink-0" />
            ...
            {SITES.map((site) => {
              ...
              return (
                <button
                  ...
                  className={`relative text-left bg-white border rounded-xl px-4 py-3.5 transition-all group ${
                    isSelected
                      ? 'border-[#1A8A9E] ring-1 ring-[#1A8A9E]/40'
                      : 'border-[#E8E5E0] hover:border-[#B0AAA0]'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[#1A1915] font-medium text-sm leading-snug">{site}</span>
                    <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${dotColor}`} />
                  </div>
                  <div className="mt-1.5 text-xs text-[#B0AAA0]">
                    {last ? (
                      <span>Last: {formatDateTime(last.logged_at)}</span>
                    ) : (
                      <span className="text-[#B0AAA0]">Never used</span>
                    )}
                  </div>
                  {isSelected && (
                    <div className="absolute top-2 right-2">
                      <X className="w-3.5 h-3.5 text-[#1A8A9E]" />
                    </div>
                  )}
                </button>
              )
      ...
      {selectedSite && (
        <div className="bg-white border border-[#1A8A9E]/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-semibold text-[#1A1915] flex items-center gap-2">
              <Plus className="w-4 h-4 text-[#1A8A9E]" />
              Log Injection —{' '}
              <span className="text-[#1A8A9E]">{selectedSite}</span>
            </h2>
            <button
              onClick={closeForm}
              className="p-1.5 text-[#B0AAA0] hover:text-[#1A1915] hover:bg-[#F2F0ED] rounded-lg transition-colors"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-[#3A3730] mb-2">
                  Peptide Name <span className="text-[#B0AAA0]">(optional)</span>
                </label>
                <input
                  type="text"
                  value={formPeptide}
      ...
      {/* ── Rotation History ── */}
      <div>
        <h2 className="text-sm font-semibold text-[#B0AAA0] uppercase tracking-wider mb-3 flex items-center gap-2">
          <Clock className="w-4 h-4" />
          Rotation History
          {recentLogs.length > 0 && (
            <span className="text-[#B0AAA0] font-normal">— last {recentLogs.length}</span>
          )}
        </h2>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-[#E8E5E0] rounded-xl p-10 text-center">
            <MapPin className="w-10 h-10 text-[#B0AAA0] mx-auto mb-3" />
            <p className="text-[#B0AAA0]">No injections logged yet.</p>
            <p className="text-[#B0AAA0] text-sm mt-1">
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-[17px] top-3 bottom-3 w-px bg-[#F2F0ED]" />

            <div className="space-y-3">
              {recentLogs.map((log) => (
                <div key={log.id} className="flex gap-4 items-start">
                  {/* Timeline dot */}
                  <div className="w-8 h-8 rounded-full bg-white border border-[#D0CCC6] flex items-center justify-center shrink-0 relative z-10">
                    <MapPin className="w-3.5 h-3.5 text-[#1A8A9E]" />
                  </div>

                  <div className="flex-1 bg-white border border-[#E8E5E0] rounded-xl px-4 py-3 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      {/* Site chip */}
                      <span className="inline-flex items-center gap-1.5 bg-[#1A8A9E]/12 border border-[#1A8A9E]/30 text-[#1A8A9E] text-xs font-medium px-2.5 py-0.5 rounded-full">
                        <MapPin className="w-3 h-3" />
                        {log.site}
                      </span>
                      {log.peptide_name && (
                        <span className="text-sm text-[#3A3730] font-medium">
                          {log.peptide_name}
                        </span>
                      )}
                    </div>

                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-[#B0AAA0]">
                    ...
                    {log.notes && (
                      <p className="mt-1.5 text-xs text-[#B0AAA0] leading-relaxed">
                        {log.notes}
                      </p>
                    )}
                  </div>
