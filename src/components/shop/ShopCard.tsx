...
export function ShopCard({ card }: { card: Card }) {
  return (
    // The card is a div, not a link: the Buy button cannot sit inside an <a>.
    // Everything above the button is the link to the product page.
    <div
      className="shop-card"
      style={{
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: "auto", paddingTop: 24 }}>
          {card.state === "assayed" && !card.isBlend && (
            <>
              ...
              >
                ...
                <span style={{ color: "#3B4045" }}>
                  {card.labelled} mg labelled
                </span>
              </div>
            </>
          )}
          ...
          >
            ...
            <span style={{ ...KICKER, textAlign: "right" }}>
              {card.dates ? "Mfg · Exp" : ""}
            </span>
            <span>{card.lot}</span>
            <span style={{ textAlign: "right", color: "#3B4045" }}>
              {card.dates}
            </span>
          </div>
          ...
          {card.state === "pending" && (
            <div style={{ marginTop: 10, borderTop: RULE, paddingTop: 10 }}>
              ...
              >
                Assay commissioned. This product ships with no published assay
                until then.
              </p>
            </div>
          )}
      ...
      <AddToCart slug={card.slug} buyNow />
    </div>
  );
}
